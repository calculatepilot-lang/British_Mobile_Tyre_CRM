...
if (preg_match('#^/leads/([^/]+)$#', $path, $m)) {
    ...
    $fields=['customer_name'=>'Customer name','customer_phone'=>'Phone','customer_email'=>'Email','service_requested'=>'Service requested','tyre_size'=>'Tyre size','vehicle_registration'=>'Vehicle registration','city'=>'City','postcode'=>'Postcode','quoted_amount'=>'Quoted amount','final_revenue'=>'Final revenue','quality_score'=>'Quality score 0-100','outcome_reason'=>'Outcome reason'];
...
if ($path === '/leads') {
    $rows=''; foreach($leads->list(100) as $lead){$rows.='<tr><td><a href="/leads/'.rawurlencode($lead['public_id']).'">'.h($lead['public_id']).'</a></td><td>'.statusBadge($lead['status']).'</td><td>'.h($lead['lead_type']).'</td><td>'.h($lead['customer_name']?:'—').'</td><td>'.h($lead['tyre_size']?:'—').'</td><td>'.h($lead['city']?:'—').'</td><td>'.h($lead['campaign_name']?:'—').'</td><td>'.h($lead['created_at']).'</td></tr>';}
    render('Leads','<div class="toolbar"><h1>Leads</h1><a class="button" href="/leads/new">Add lead</a></div><table><thead><tr><th>ID</th><th>Status</th><th>Type</th><th>Customer</th><th>Tyre size</th><th>City</th><th>Campaign</th><th>Created</th></tr></thead><tbody>'.($rows?:'<tr><td colspan="8">No leads yet.</td></tr>').'</tbody></table>',$user);
}
